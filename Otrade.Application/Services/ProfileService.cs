...
public class ProfileService
{
    ...
    public ProfileService(OtradeDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<ApiResponse<object>> GetProfileAsync(long userId)
    {
        ...
        var walletAddress = await _context.UserWalletAddresses
            .FirstOrDefaultAsync(x => x.UserId == userId);
        ...
        return ResponseFactory.Success<object>(new
        {
            ...
            WalletAddress = walletAddress == null ? null : new
            {
        ...
        ChangePasswordRequest request)
    {
        if (string.IsNullOrWhiteSpace(request.CurrentPassword))
            return ResponseFactory.Fail<bool>("Current password is required");

        if (string.IsNullOrWhiteSpace(request.NewPassword))
            return ResponseFactory.Fail<bool>("New password is required");

        if (request.NewPassword.Length < 6)
            return ResponseFactory.Fail<bool>("New password must be at least 6 characters");
        ...
        if (!isValid)
            return ResponseFactory.Fail<bool>("Current password is incorrect");
